...
const ZINEWINDOW = document.querySelector('#zine')
let card_amount;
let current_state = 0;
let textures = [];
let pages = [];

ZINEWINDOW.style.background = BGCOLOR;
ZINEWINDOW.ariaLabel = ALT;
if (SMOOTH) {
    ZINEWINDOW.style.imageRendering = 'auto';
} else {
    ZINEWINDOW.style.imageRendering = 'pixelated';
}
...
function getTextures(num) {
    return [`${FOLDER}FRONT.png`, `${FOLDER}INNERFRONT.png`].concat(
        new Array(num).fill().map((_, idx) => FOLDER + (idx + 1) + '.png'),
        [`${FOLDER}BACK.png`]
    );
}
...
function startZine() {
    ...
    // Keyboard input
    ZINEWINDOW.addEventListener('keyup', function onKeyUp(key) {
    ...
    // Mouse input
    ZINEWINDOW.addEventListener('pointerup', function onPointerUp(event) {
